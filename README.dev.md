...
# install simit as an editable package
...
# recursively check import style for the simit module only
isort --recursive --check-only simit

# recursively check import style for the simit module only and show
# any proposed changes as a diff
isort --recursive --check-only --diff simit

# recursively fix import style for the simit module only
isort --recursive simit
```
...
cd $(mktemp -d --tmpdir simit.XXXXXX)

# fresh git clone ensures the release has the state of origin/main branch
git clone https://github.com/fdiblen/simit .
...
Visit
[https://test.pypi.org/project/simit](https://test.pypi.org/project/simit)
and verify that your package was uploaded successfully. Keep the terminal open, we'll need it later.

In a new terminal, without an activated virtual environment or an env directory:

```shell
cd $(mktemp -d --tmpdir simit-test.XXXXXX)
...
--extra-index-url https://pypi.org/simple simit
```
